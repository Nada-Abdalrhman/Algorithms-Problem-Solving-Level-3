...
class Program
{
    ...
    public static int ShowTransactionMenu()
    {
        Console.WriteLine($"============================================");
        Console.WriteLine($"{"Transaction Menu Screen".PadLeft(33)}");
        Console.WriteLine($"============================================");
    ...
    public static void ShowTotalBalances(List<Client> clients)
    {
        Console.Clear();
        double TotalBalance = 0;
        if (clients.Count != 0)
        {
            Console.WriteLine($"Balance List ({clients.Count}) Client(s)".PadLeft(55));
            Console.WriteLine($"------------------------------------------------------------------------------------------");
            Console.WriteLine($"| {"Account Number".PadRight(15)}| {"Client Name".PadRight(25)}| {"Balance".PadRight(13)}");
            Console.WriteLine($"------------------------------------------------------------------------------------------");
            foreach (var c in clients)
            {
                TotalBalance += c.Balance;
                Console.WriteLine($"| {c.AccountNumber.PadRight(15)}| {c.ClientName.PadRight(25)}| {c.Balance.ToString().PadRight(13)}");
            }
            Console.WriteLine($"------------------------------------------------------------------------------------------");
            Console.WriteLine($"Total Balance = {TotalBalance}".PadLeft(53));
        }
        else
        {
            Console.WriteLine($"No Clients Added yet!");
    ...
    public static void BackToTransactions()
    {
    ...
    public static double ReadAmount(string text)
    {
        Console.Write($"{text}");
        double amount = double.Parse(Console.ReadLine());
        return amount;
    }
    public static void DepositScreen(string path, ref List<Client> clients)
    {
        Console.Clear();
        Console.WriteLine($"-----------------------------------");
        Console.WriteLine($"Deposit Screen".PadLeft(23));
        Console.WriteLine($"-----------------------------------");
        bool Found = false;
        string AccNum = "";
        do
        ...
        Console.WriteLine("\n");
        int index = clients.IndexOf(client);
        double deposite = ReadAmount("Please Enter deposit amount ? ");
        Console.Write($"\n\nAre you sure you want to perform this transaction? y/n ? ");
        char key = Console.ReadKey().KeyChar;
        if (key == 'y' || key == 'Y')
        {
            client.Balance += deposite;
            UpdateClientInFile(path, client, index);
            clients[index] = client;
            StartApp(6);
        }
    ...
    public static void WithdrawScreen(string path, ref List<Client> clients)
    {
        Console.Clear();
        Console.WriteLine($"-----------------------------------");
        Console.WriteLine($"Withdraw Screen".PadLeft(23));
        Console.WriteLine($"-----------------------------------");
        bool Found = false;
        string AccNum = "";
        do
        ...
        Console.WriteLine("\n");
        int index = clients.IndexOf(client);
        double withdraw = ReadAmount("Please Enter withdraw amount ? ");
        while(withdraw > client.Balance)
        {
            Console.WriteLine($"Amount Exceeds the balance, you can withdraw up to : {client.Balance}");
            withdraw = ReadAmount($"Please enter another amount? ");
        }
        Console.Write($"\n\nAre you sure you want to perform this transaction? y/n ? ");
        char key = Console.ReadKey().KeyChar;
        if (key == 'y' || key == 'Y')
    ...
    public static void HandleTransactions(string path, ref List<Client> clients)
    {
        Console.Clear();
        int choose = ShowTransactionMenu();
        switch (choose)
        {
            case 1:
                DepositScreen(path, ref clients);
                break;
            case 2:
                WithdrawScreen(path, ref clients);
                break;
            case 3:
    ...
    public static void StartApp(int? choose = null)
    {
        Console.Clear();
        ...
        switch (choose)
        {
            ...
            case 5:
                ...
                break;
            case 6:
                HandleTransactions(path, ref clients);
                break;
            case 7:
                EndApp();
                break;
