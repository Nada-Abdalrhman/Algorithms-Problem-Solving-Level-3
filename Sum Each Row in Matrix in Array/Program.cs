namespace Sum_Each_Row_in_Matrix_in_Array;
...
class Program
{
    static void Main(string[] args)
    {
        ...
        PrintSumOfEachRow($"The following are the sum of each row in the matrix: ", CalcSumOfEachRow(arr));
        Console.WriteLine("--------------------");
        var arr2 = FillArr(9, 9, 5, 500);
        PrintSumOfEachRow($"The following are the sum of each row in the matrix: ", CalcSumOfEachRow(arr2));
    ...
    public static int[] CalcSumOfEachRow(int[,] arr)
    {
    ...
    public static void PrintSumOfEachRow(string text, int[] arr)
    {
        ...
        {
            Console.WriteLine($"Row {i + 1} Sum = {arr[i]}");
        }
    }
}
